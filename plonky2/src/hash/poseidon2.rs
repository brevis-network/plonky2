...
use crate::gates::poseidon2_gate::Poseidon2Gate;
...
pub const WIDTH: usize = 8; // we only have width 8 and 12, and 12 is bigger. :)
...
pub trait Poseidon2: PrimeField64 {
        ...
        Self: RichField + Extendable<D>,
    {
        ...
        result[0] = builder.add_many_extension([input[0], input[0], input[4]]);
        result[1] = builder.add_many_extension([input[1], input[1], input[5]]);
        result[2] = builder.add_many_extension([input[2], input[2], input[6]]);
        result[3] = builder.add_many_extension([input[3], input[3], input[7]]);

        result[4] = builder.add_many_extension([input[0], input[4], input[4]]);
        result[5] = builder.add_many_extension([input[1], input[5], input[5]]);
        result[6] = builder.add_many_extension([input[2], input[6], input[6]]);
        result[7] = builder.add_many_extension([input[3], input[7], input[7]]);
